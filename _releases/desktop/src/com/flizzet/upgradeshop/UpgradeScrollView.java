...
public class UpgradeScrollView extends GuiComponent {
	...
	this.scrollWindowImage = GameManager.getInstance().assetManager.get("gui/upgradeShop/scrollWindow.png");
	...
	scrollWindowX = bounds.x + (MainCamera.getInstance().getWidth() / 2) - (scrollWindowImage.getWidth() / 2);
	scrollWindowY = bounds.y + (MainCamera.getInstance().getHeight() / 2) - (scrollWindowImage.getHeight() / 2);
	...
	/* Handling a scroll */
	initialMouseY += (MainCamera.getInstance().getMousePos().y - initialMouseY) / 5;
	if (Gdx.input.isTouched()) {
	    ...
	    mouseChange = initialMouseY - MainCamera.getInstance().getMousePos().y;
	} else {
	    pushed = false;
	}
	...
	/* Stopping at ceiling */
	// TODO Make ceiling and floor based on scrollview vs camera when more are added
	...
	/* Stopping at floor */
	if (cells.get(0).getY() + cells.get(0).getHeight() < scrollWindowY + scrollWindowImage.getHeight()) {
	    if (Math.signum(mouseChange) == 1) {
		yChange -= mouseChange / 5;
	    }
	}
    ...
    public float getWindowX()			{ return this.scrollWindowX; }
    public float getWindowY()			{ return this.scrollWindowY; }
    public float getWindowWidth()		{ return this.scrollWindowImage.getWidth(); }
    public float getWindowHeight()		{ return this.scrollWindowImage.getHeight(); }
    public int getState()			{ return this.state; }
    public UpgradeMenuBuilder getMenuBuilder()	{ return this.menuBuilderPassed; }
