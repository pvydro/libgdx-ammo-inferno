...
public class SettingsButton extends ButtonComponent {
	...
	this.setImage(GameManager.getInstance().assetManager.get("gui/pauseMenu/settingsButton.png", Texture.class));
	this.setImageHovered(GameManager.getInstance().assetManager.get("gui/pauseMenu/settingsButton.png", Texture.class));
	this.setImagePushed(GameManager.getInstance().assetManager.get("gui/pauseMenu/settingsButtonPushed.png", Texture.class));
    }
	...
	if (isPushed()) {
	    batch.draw(shineImage, bounds.x + 2 + shakeXOffset, subtextY + 1 + 2 + shakeYOffset, bounds.width - 4, bounds.height - 4);
	} else {
	    batch.draw(shineImage, bounds.x + shakeXOffset, subtextY + 1 + shakeYOffset, bounds.width, bounds.height);
	}
    }
