...
public class SawEnemyCollision extends EnemyCollision implements Renderable {
	...
	playerBounds = ShapeUtils.rectToPolygon(Player.getInstance().getCollisionBounds());
	...
	playerBounds.setPosition(Player.getInstance().getX(), Player.getInstance().getY());
	...
	if (Intersector.overlapConvexPolygons(collisionBounds, playerBounds)) {
		...
		GameManager.getInstance().particleFunctions.addBloodParticle(playerBounds.getX(), playerBounds.getY(), 
