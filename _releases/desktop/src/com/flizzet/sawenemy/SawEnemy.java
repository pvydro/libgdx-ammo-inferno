...
public class SawEnemy extends Enemy {
	...
	image = GameManager.getInstance().assetManager.get("enemies/rollingSaw/saw.png");
	adjustBoundsToImage(image);
	
	this.collision = new SawEnemyCollision(this);		// Create after creation for bounds
	...
	this.setY(platform.getY() + platform.getHeight() - (this.getWidth() / 2) + PlatformGenerator.getInstance().getSpeed());
	...
	batch.draw(image, bounds.x, bounds.y, originX, originY, bounds.width, bounds.height, 1, 1, rotation, 0, 0,
