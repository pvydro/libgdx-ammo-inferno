...
public class FlyingEyeEnemy extends FlyingEnemy {
    ...
    private FlyingEyeEnemyCollision collision = new FlyingEyeEnemyCollision(this);
	...
	animation = AnimationUtils.newAnimation(GameManager.getInstance().assetManager.get("enemies/flyingEye/flyingEye.png", Texture.class), 8, 1, 300);
	...
	/* Flip the eye direction to
	 * 1: Face the platform it's chasing
	 * 2: Face the player  */
	if (getAi().getState() == FlyingEnemyState.CHASING_PLATFORM) {
	    if (getAi().getTargetPlatform().getSide() == 1 || getAi().getTargetPlatform().getSide() == 2) {
	...
	if (CurrentSettings.getInstance().blood) {
	    GameManager.getInstance().particleFunctions.addBloodParticle(getCenterX(), getCenterY(),
    ...
    public FlyingEnemyAi getAi()	{ return (FlyingEnemyAi) this.ai; }
