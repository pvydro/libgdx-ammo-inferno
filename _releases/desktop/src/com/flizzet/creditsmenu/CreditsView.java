...
public class CreditsView extends GuiComponent {
    ...
    private GlyphLayout programmedByLayout = new GlyphLayout(smallFont, programmedBy);
    ...
    private GlyphLayout migrainesByLayout = new GlyphLayout(smallFont, migrainesBy);
    private float migrainesByX, migrainesByY;
    
    /* Sleepless nights and */
    private String sleeplessNightsBy = "sleepless nights and";
    private GlyphLayout sleeplessNightsLayout = new GlyphLayout(smallFont, sleeplessNightsBy);
    ...
    private GlyphLayout monetizationByLayout = new GlyphLayout(smallFont, monetizationBy);
    private float monetizationByX, monetizationByY;
    
    /* Maintenance and */
    private String maintenanceBy = "maintenance and";
    private GlyphLayout maintenanceByLayout = new GlyphLayout(smallFont, maintenanceBy);
    private float maintenanceByX, maintenanceByY;
    
    /* Store design and */
    private String storeDesignBy = "store design and";
    private GlyphLayout storeDesignByLayout = new GlyphLayout(smallFont, storeDesignBy);
    private float storeDesignByX, storeDesignByY;
    
    /* Distribution by */
    private String distributionBy = "distribution by";
    private GlyphLayout distributionByLayout = new GlyphLayout(smallFont, distributionBy);
    ...
    /* LibGDX */
    private Texture libgdxLogo = GameManager.getInstance().assetManager.get("gui/creditsMenu/libgdxLogo.png");
    private float libgdxX, libgdxY;
    
    /* Separator */
    private SeparatorComponent firstSeparator = new SeparatorComponent(2);
    private SeparatorComponent contactSeparator = new SeparatorComponent(2, "contact");
    private SeparatorFillerComponent filler = new SeparatorFillerComponent(firstSeparator, contactSeparator);
	...
	sleeplessNightsY = migrainesByY - migrainesByLayout.height - STACK_MARGIN;
	
	/* Design and */
	designByX = MARGIN;
	designByY = sleeplessNightsY - sleeplessNightsLayout.height - STACK_MARGIN;
	...
	maintenanceByY = monetizationByY - monetizationByLayout.height - STACK_MARGIN;
	
	/* Store design and */
	storeDesignByX = MARGIN;
	storeDesignByY = maintenanceByY - maintenanceByLayout.height - STACK_MARGIN;
	
	/* Distribution by */
	distributionByX = MARGIN;
	distributionByY = storeDesignByY - storeDesignByLayout.height - STACK_MARGIN;
	...
	/* In Java with */
	inJavaX = MainCamera.getInstance().getWidth() - MARGIN - inJavaLayout.width;
	inJavaY = flizzetY - flizzetLayout.height - STACK_MARGIN;
	
	/* LibGDX */
	libgdxX = MainCamera.getInstance().getWidth() - MARGIN - libgdxLogo.getWidth();
	libgdxY = inJavaY - inJavaLayout.height - STACK_MARGIN - libgdxLogo.getHeight();
	
	/* Separate */
	contactSeparator.setY(libgdxY - libgdxLogo.getHeight() - STACK_MARGIN - 20);
	...
	for (ButtonComponent b : contactButtons) {
	    ...
	    b.setY(contactSeparator.getY() - b.getHeight() - (STACK_MARGIN * 2));
	    b.update(delta);
	    totalButtons++;
	}
    }
	...
	/* Sleepless nights and */
	smallFont.draw(batch, sleeplessNightsBy, sleeplessNightsX, sleeplessNightsY);
	/* Design and */
