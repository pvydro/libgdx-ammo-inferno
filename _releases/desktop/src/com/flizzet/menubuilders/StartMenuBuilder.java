...
public class StartMenuBuilder extends MenuBuilder {
	...
	/* Position buttons */
	int totalButtons = 0;
	float fullWidth = (buttons.get(0).getWidth()) * buttons.size() + (STACK_MARGIN * (buttons.size() - 1));
	...
	/* Title */
	title.setX(MainCamera.getInstance().getCenterX() - (title.getWidth() / 2));
	title.setY(MainCamera.getInstance().getHeight() - title.getHeight() - MARGIN);
