...
public class PlayerController implements Updatable {
	...
	this.player.setX(GameManager.getInstance().getCamera().getCenterX() - (player.getWidth() / 2));
	...
	if (targetPlatform != null && player.getY() == targetPlatform.getY() + targetPlatform.getHeight() && !jumping && jumpCooldown <= 0) {
	    totalJumps = 0;
	}
	...
	if (CurrentSettings.getInstance().controls == ControlType.CONSOLE) {
	    if (InGameGui.getInstance().getTouchpad().getTouchpad().getKnobPercentX() > 0) {
		rightPushed = true;
	    } else if (InGameGui.getInstance().getTouchpad().getTouchpad().getKnobPercentX() < 0) {
		leftPushed = true;
	    }
	}
	...
	if (upPushed && colliding && !jumping && jumpCooldown <= 0
		&& Player.getInstance().getY() < MainCamera.getInstance().getHeight()) {
	    yVelocity = 0;
	    jump();
	}
	/* Double jumping */
	if (upPushed && totalJumps < 1 && Upgrades.getInstance().isEquipped(DoubleJumpUpgrade.class)) {
	    if (yVelocity < 0) {
		jump();
	    }
	}
	...
	addGroundParticle();
	if (CurrentSettings.getInstance().controls == ControlType.CONSOLE) {
	    xVelocity += ((topXVelocity * InGameGui.getInstance().getTouchpad().getTouchpad().getKnobPercentX()) - xVelocity) / 3;
	}
    }
    ...
    /** When landing, the player will sink within the floor with his speed
     * 	This function fixes the sink
     * 	No plumber */
	...
	if (!jumping && groundParticleCooldown <= 0) {
		...
		GameManager.getInstance().particleFunctions.addGroundParticle(player.getCenterX(), player.getY(),
			new Random().nextInt(CurrentSettings.getInstance().groundParticleAmount) + 1);
	    }
	    groundParticleCooldown = CurrentSettings.getInstance().groundParticleTotal;
	}
    }
    ...
    public float getYVel() 					{ return this.yVelocity; }
    public float getXVel()					{ return this.xVelocity; }
    public boolean isColliding()				{ return this.colliding; }
    public boolean isJumping()					{ return this.jumping; }
    public Platform getTargetPlatform()				{ return this.targetPlatform; }
    
    public void setRightPushed(boolean isRightPushed)		{ this.rightPushed = isRightPushed; }
    public void setLeftPushed(boolean isLeftPushed)		{ this.leftPushed = isLeftPushed; }
    public void setUpPushed(boolean isUpPushed)			{ this.upPushed = isUpPushed; }
    public void setXVel(float newXVelocity)			{ this.xVelocity = newXVelocity; }
    public void setYVel(float newYVelocity) 			{ this.yVelocity = newYVelocity; }
    public void setColliding(boolean isColliding)		{ this.colliding = isColliding; }
    public void setDead(boolean isDead)				{ this.dead = isDead; }
    public void setJumping(boolean isJumping)			{ this.jumping = isJumping; }
    public void setJumpCooldown(int newJumpCooldown)		{ this.jumpCooldown = newJumpCooldown; }
	...
	if (!newTargetPlatform.isHit()) {
	    ...
	    PlatformIndicator.getInstance().setTargetPlatform(newTargetPlatform);
	}
    }
