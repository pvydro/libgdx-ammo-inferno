...
public class Weapon extends Entity {
	...
	/* Muzzle Position */
	float mx = (float) (muzzleLength * Math.cos(rotRad) - muzzleHeight * Math.sin(rotRad));
	float my = (float) (muzzleLength * Math.sin(rotRad) + muzzleHeight * Math.cos(rotRad));
	...
	for (Particle p : GameManager.getInstance().particleContainer.getParticles()) {
		...
		p.setX(muzzleX);
		p.setY(Player.getInstance().getDirection() < 1 ? muzzleY : muzzleY + 5);
	...
	if (direction == 1) {
	    batch.draw(gold ? imageGold : image, bounds.x - recoilX, bounds.y, originX, originY, bounds.width, bounds.height, 1, 1, rotation + recoilRotation, 0, 0,
		    (int) bounds.width, (int) bounds.height, false, false);
	} else {
	    batch.draw(gold ? imageGold : image, bounds.x + recoilX, bounds.y, originX, originY, bounds.width, bounds.height, 1, 1, rotation - recoilRotation, 0, 0,
		    (int) bounds.width, (int) bounds.height, false, true);
	}
    }
	...
	if (CurrentSettings.getInstance().mouseAim) {
	    float angle = (float) Math.atan2(MainCamera.getInstance().getMousePos().y - bounds.y,
		...
		if (!target.isDead()) {
		    /* Point to target */
		    float angle = (float) Math.atan2((target.getY() + PlatformGenerator.getInstance().getSpeed()) - bounds.y, target.getX() - bounds.x);
	...
	/* Play shoot sound */
	Sounds.play(Sounds.getInstance().shootSound, 0.4f, 0.8f + random.nextFloat());
	/* Randomly play shell sound */
	if (random.nextInt(10) == 5) {
	    Sounds.play(Sounds.getInstance().shellSound, 0.5f, 1.0f + random.nextFloat());
	}
	if (random.nextInt(40) == 9) {
	    Sounds.play(Sounds.getInstance().reloadSound, 1.0f, 0.8f + random.nextFloat());
	}
	...
	/* Adding particles */
	GameManager.getInstance().particleFunctions.addMuzzleFlash(muzzleX, muzzleY, rotation);
	...
	if (CurrentSettings.getInstance().screenFlash) {
	    GameManager.getInstance().particleFunctions.addScreenFlash(Math.abs(strength) / 5);
	}
    ...
    public void setGold(boolean isGold)		{ this.gold = isGold; }
    public void setTarget(Enemy newTarget)	{ this.target = newTarget; }
    public void setTotalKills(int newTotalKills){ this.totalKills = newTotalKills; }
    public void setFireRate(int newFireRate)	{ this.fireRate = newFireRate; }
    
    public Texture getImageNoHand()		{ return this.imageNoHand; }
    public Texture getImageGoldNoHand()		{ return this.imageGoldNoHand; }
    ...
    public int getKillsToGold()			{ return this.killsToGold; }
