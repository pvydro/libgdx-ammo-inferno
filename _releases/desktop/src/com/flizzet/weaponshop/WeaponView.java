...
class WeaponView extends GuiComponent {
	...
	if (selected) {
	    if (textAlpha < 1) textAlpha += (0.05f);
	...
	/* Price */
	priceX = MainCamera.getInstance().getCenterX() - (priceLayout.width / 2);
	priceY = nameY - nameLayout.height - 2;
	/* Total kills */
	totalKillsX = MainCamera.getInstance().getCenterX() - (totalKillsLayout.width / 2);
	...
	goldButton.setX(MainCamera.getInstance().getCenterX() - (goldButton.getWidth() / 2));
	if (weapon.getTotalKills() >= weapon.getKillsToGold()) {
	    goldButton.setY(totalKillsY - priceLayout.height - goldButton.getHeight() - 12);
	...
	batch.setColor(new Color(tmp.r - alpha, tmp.g - alpha, tmp.b - alpha, 1.0f));
	
	if (weapon.isGold()) {
	    batch.draw(imageGold, view.getX() + bounds.x, view.getY() + bounds.y + yChange, imageWidth, imageHeight);
	} else {	    
	    batch.draw(image, view.getX() + bounds.x, view.getY() + bounds.y + yChange, imageWidth, imageHeight);
	}
	batch.setColor(tmp);
	...
	if (weapon.isBought()) {
	    ...
	    this.price = "bought";
	    this.totalKills = String.valueOf(weapon.getTotalKills()) + "/" + String.valueOf(weapon.getKillsToGold());
	} else {
	    this.name = "?";
