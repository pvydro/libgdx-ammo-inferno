...
public class WeaponStand extends GuiComponent {
	...
	this.image = GameManager.getInstance().assetManager.get("gui/weaponShop/stand.png", Texture.class);
	this.holoImage = GameManager.getInstance().assetManager.get("gui/weaponShop/holo.png", Texture.class);
	...
	buyButton.setWeapon(view.getViews().get(view.getCurrentSelection()).getWeapon());
	
	/* Position buy button */
	buyButton.setX(MainCamera.getInstance().getCenterX() - (buyButton.getWidth() / 2));
	buyButton.setY(20);
	...
	newParticle.setX(this.getX() + 4 + random.nextInt((int) this.getWidth() - 8) - newParticle.getWidth());
	newParticle.setY(this.getY() + this.getHeight() - newParticle.getHeight());
