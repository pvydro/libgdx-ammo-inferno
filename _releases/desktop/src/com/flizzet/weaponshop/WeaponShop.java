...
public class WeaponShop extends GuiComponent {
	...
	backButton.setX(MainCamera.getInstance().getCenterX() - (totalWidth / 2));
	backButton.setY(MainCamera.getInstance().getHeight() - MARGIN - backButton.getHeight());
	backButton.update(delta);
	
	/* Header position */
	headerX = MainCamera.getInstance().getCenterX() + (totalWidth / 2) - headerLayout.width;
	headerY = backButton.getY() + (backButton.getHeight()) - (headerLayout.height / 2);
