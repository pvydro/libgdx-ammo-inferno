...
public class GunTrapEnemy extends Enemy {
	...
	this.pad = new GunTrapEnemyPad(this);		// Create in constructor because it gets the platform
	
	if (Upgrades.getInstance().isEquipped(TrapJammerUpgrade.class)) {
	    this.image = GameManager.getInstance().assetManager.get("enemies/gunTrap/shooterJammed.png");
	} else {
	    this.image = GameManager.getInstance().assetManager.get("enemies/gunTrap/shooter.png", Texture.class);
	}
	...
	if (platform.getSide() == 0) {		// Left
	    ...
	    this.setX(platform.getX() + platform.getWidth() - 20 - this.getWidth());
	}
    ...
    public int getSide()		{ return this.platform.getSide(); }
