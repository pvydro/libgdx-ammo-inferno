...
public class ShopSelectionMenu extends GuiComponent {
    ...
    private float headerX = MainCamera.getInstance().getCenterX() - (headerLayout.width / 2) + 400;
    private float headerY = MainCamera.getInstance().getHeight() - headerLayout.height - MARGIN;
    private float backButtonY = headerY - (fontHeader.getCapHeight() / 2) - (backButton.getHeight() / 2);
    ...
    public void enable() {
	builder.buildMenu();
	headerX = MainCamera.getInstance().getCenterX() - (headerLayout.width / 2) + 400;
    ...
    public ShopSelectionMenuBuilder getBuilder()	{ return this.builder; }
