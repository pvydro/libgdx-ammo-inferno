...
public class RewardPrompt extends GuiComponent {
    ...
    private String totalDays = String.valueOf(CurrentSettings.getInstance().totalDays)
	    + (CurrentSettings.getInstance().totalDays > 1 ? " days" : " day");
    private GlyphLayout totalDaysLayout = new GlyphLayout(totalDaysFont, totalDays);
    ...
    private String score = "+" + DailyReward.getInstance().getChecker().getReward();
	...
	this.bgImage = GameManager.getInstance().assetManager.get("gui/dailyReward/promptBg.png");
	this.rewardIcon = GameManager.getInstance().assetManager.get("gui/dailyReward/rewardIcon.png");
	...
	headerY = this.getY() + this.getHeight() - (headerLayout.height) - STACK_MARGIN;
	...
	/* Position score */
	float fullWidth = rewardIcon.getWidth() + STACK_MARGIN + scoreLayout.width;
	...
	rewardIconY = scoreY - (scoreLayout.height / 2) - (rewardIcon.getHeight() / 2);
...
class RewardAcceptButton extends ButtonComponent {
    ...
    private Texture shineImage = GameManager.getInstance().assetManager.get("gui/dailyReward/buttonShine.png", Texture.class);
	...
	this.setImage(GameManager.getInstance().assetManager.get("gui/dailyReward/acceptButton.png", Texture.class));
	this.setImageHovered(GameManager.getInstance().assetManager.get("gui/dailyReward/acceptButton.png", Texture.class));
	this.setImagePushed(GameManager.getInstance().assetManager.get("gui/dailyReward/acceptButtonPushed.png", Texture.class));
    }
