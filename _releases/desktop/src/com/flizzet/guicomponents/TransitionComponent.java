...
public class TransitionComponent extends GuiComponent {
	...
	/* Setting images */
	leftImage = GameManager.getInstance().assetManager.get("gui/transition/leftDoor.png", Texture.class);
	rightImage = GameManager.getInstance().assetManager.get("gui/transition/rightDoor.png", Texture.class);
	...
	float leftSide = newCenterX - (fullWidth / 2);	// Set the left side to the edge of the new center
	float rightSide = newCenterX + (fullWidth / 2);	// Set the right side to the edge of the new center
	startLeftX = leftSide - leftWidth;		// Set start position of the left door
	...
	leftTargetX = newCenterX - leftWidth;		// Set the position eased into by the left door
	rightTargetX = newCenterX;			// Set the position eased into by the right door
	...
	if (state == 0) {
		...
		GameManager.getInstance().transitionContainer.removeFromTransitions(this);
	    }
	}
