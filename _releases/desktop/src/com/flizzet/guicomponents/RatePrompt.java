...
public class RatePrompt extends GuiComponent {
	...
	this.bgImage = GameManager.getInstance().assetManager.get("gui/rateReward/promptBg.png");
	this.giftIcon = GameManager.getInstance().assetManager.get("gui/rateReward/giftIcon.png");
	adjustBoundsToImage();
	...
	/* Position background */
	bounds.x = MainCamera.getInstance().getCenterX() - (bgImage.getWidth() / 2);
	bounds.y += (MainCamera.getInstance().getCenterY() - (bgImage.getHeight() / 2) - bounds.y) / 5;
	
	/* Position header */
	headerX = getCenterX() - (headerLayout.width / 2);
	headerY = bounds.y + bounds.height - (headerLayout.height) - STACK_MARGIN;
	...
	/* Position buttons */
	float fullWidth = acceptButton.getWidth() + declineButton.getWidth() + STACK_MARGIN;
	acceptButton.setX(getCenterX() + (fullWidth / 2) - acceptButton.getWidth());
...
class RateAcceptButton extends ButtonComponent {
	...
	this.setImage(GameManager.getInstance().assetManager.get("gui/rateReward/acceptButton.png", Texture.class));
	this.setImageHovered(GameManager.getInstance().assetManager.get("gui/rateReward/acceptButton.png", Texture.class));
	this.setImagePushed(GameManager.getInstance().assetManager.get("gui/rateReward/acceptButtonPushed.png", Texture.class));
    }
    ...
    @Override
    public void triggered() {
	Gdx.net.openURI("https://play.google.com/store/apps/details?id=com.flizzet.main");
	GameManager.getInstance().guiContainer.removeFromGui(prompt);
	GameNotification noti = new GameNotification("+$100");
	Player.getInstance().getScore().setCoins(Player.getInstance().getScore().getTotalCoins() + 100);
...
class RateDeclineButton extends ButtonComponent {
    ...
    private Texture shineImage = GameManager.getInstance().assetManager.get("gui/rateReward/buttonShine.png", Texture.class);
    private RatePrompt prompt;
	...
	this.setImage(GameManager.getInstance().assetManager.get("gui/rateReward/declineButton.png", Texture.class));
	this.setImageHovered(GameManager.getInstance().assetManager.get("gui/rateReward/declineButton.png", Texture.class));
	this.setImagePushed(GameManager.getInstance().assetManager.get("gui/rateReward/declineButtonPushed.png", Texture.class));
    }
