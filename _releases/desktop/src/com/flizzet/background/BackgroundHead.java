...
class BackgroundHead extends Entity {
   ...
   public BackgroundHead(int scale, float alpha, BackgroundComponent background) {
	this.background = background;
	this.headImage = GameManager.getInstance().assetManager.get("enemies/zombie/head/headDefault.png");
	this.scale = scale;
	this.alpha = alpha;
   ...
   public void render(SpriteBatch batch) {
	...
	batch.setColor(new Color(tmp.r - alpha - background.getScreenFlashAmount(),
		tmp.g - alpha - background.getScreenFlashAmount(),
		tmp.b - alpha - background.getScreenFlashAmount(), 1));
	
	batch.draw(headImage, bounds.x, bounds.y, originX, originY / 2, bounds.width, bounds.height, scale, scale, rotation, 0, 0,
