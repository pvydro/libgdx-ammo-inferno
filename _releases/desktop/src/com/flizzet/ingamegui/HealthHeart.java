...
class HealthHeart extends Entity {
	...
	heartImage = GameManager.getInstance().assetManager.get("gui/inGame/heart.png", Texture.class);
	...
	if (seq == 0) stateTime += delta;
	if (seq == 1) stateTime -= delta;
	...
	batch.draw(heartImage, bounds.x, bounds.y + yChange, bounds.width, bounds.height);
