...
public class PlatformIndicator extends Entity {
	...
	image = GameManager.getInstance().assetManager.get("gui/inGame/platformIndicator.png");
	imageHigh = GameManager.getInstance().assetManager.get("gui/inGame/platformIndicatorHigh.png");
	adjustBoundsToImage();
    }
	...
	if (targetPlatform != null) {
		...
		newX = targetPlatform.getX() + targetPlatform.getWidth() + spacing;
		...
		if (!sideSet) {
		 ...
		 if (side == 0) 	newX = targetPlatform.getX() + targetPlatform.getWidth() + spacing;
		 else if (side == 1) 	newX = targetPlatform.getX() - spacing - bounds.width;
	    } else {
	...
	/* Update light */
	if (Player.getInstance().getScore().isHighScoreShown()) light.update(delta);
    }
