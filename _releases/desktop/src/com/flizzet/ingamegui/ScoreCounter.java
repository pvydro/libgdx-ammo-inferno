...
public class ScoreCounter extends GuiComponent {
	...
	this.platformIcon = GameManager.getInstance().assetManager.get("gui/icons/platformIconLower.png", Texture.class);
	...
	platformX = MARGIN;
	platformY = MainCamera.getInstance().getHeight() - MARGIN - platformHeight;
	
	/* Pause button positioning */
	pauseButton.setX(coinX + (coinWidth / 2) - (pauseButton.getWidth() / 2));
	...
	/* Draw icons */
	batch.draw(coinIcon, coinX + shakeXOffset, coinY + shakeYOffset, coinWidth, coinHeight);
	batch.draw(platformIcon, platformX + shakeXOffset, platformY + shakeYOffset, platformWidth, platformHeight);
	
	/* Draw numbers */
	/* Platforms */
	BitmapFont font = FontUtils.UPHEAVAL_SCORECOUNTER_MEDIUM;
	font.setUseIntegerPositions(false);
	font.draw(batch, String.valueOf(Player.getInstance().getScore().getPlatforms()),
		platformX + platformWidth + textSpacing + shakeXOffset,
		platformY + (platformHeight / 2) + (font.getCapHeight() / 2) + shakeYOffset);

	/* Coins */
	layout.setText(font, String.valueOf(Player.getInstance().getScore().getCoins()));
	font.draw(batch, String.valueOf(Player.getInstance().getScore().getCoins()),
		coinX - layout.width - textSpacing + shakeXOffset,
		coinY + (coinHeight / 2) + (font.getCapHeight() / 2) + shakeYOffset);
    ...
    public void setPlatformIcon(Texture newIcon)	{ this.platformIcon = newIcon; }
