...
public class JoystickComponent extends GuiComponent {
	...
	newReference = newReference + verticalReference + horizontalReference + ".png";
	/* Connect it to the image */
	currentImage = GameManager.getInstance().assetManager.get(baseReference + newReference);
    }
	...
	batch.draw(currentImage, bounds.x + shakeXOffset, bounds.y + shakeYOffset, bounds.width, bounds.height);
    }
	...
	this.setWidth(GameManager.getInstance().assetManager.get(baseReference + "joystickMid.png", Texture.class).getWidth());
	this.setHeight(GameManager.getInstance().assetManager.get(baseReference + "joystickMid.png", Texture.class).getHeight());
    }    
