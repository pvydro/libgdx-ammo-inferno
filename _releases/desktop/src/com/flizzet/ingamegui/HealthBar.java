...
public class HealthBar extends Entity {
	...
	if (heart2 == null) return;				// Stop the function if no health
	
	/* Locals */
	float center = ((target.getCenterX() - (heart2.getWidth() / 2)));						// Player center
	float heartX = heart2.getX() + 60 * ((center - heart2.getX()) / 2) * delta;					// Center base heart
	float heartY = heart2.getY() + 60 * ((target.getY() + target.getHeight() + 10 - heart2.getY()) / 2) * delta;	// Constant heart y
	...
	if (target instanceof Player) {
	    if (heart2 != null)	heart2.update(delta);
	}
	...
	if (health >= 1) heart1.render(batch);
	if (health >= 2) heart3.render(batch);
	if (health >= 3) heart2.render(batch);
	if (health >= 4) heart4.render(batch);
