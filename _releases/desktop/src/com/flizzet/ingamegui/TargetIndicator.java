...
public class TargetIndicator extends GuiComponent {
	...
	GameManager.getInstance().assetManager.get("gui/inGame/targetTopLeft.png"),	// Top Left - 		0
	GameManager.getInstance().assetManager.get("gui/inGame/targetTopRight.png"),	// Top Right - 		1
	GameManager.getInstance().assetManager.get("gui/inGame/targetBottomRight.png"),	// Bottom Right - 	2
	GameManager.getInstance().assetManager.get("gui/inGame/targetBottomLeft.png")	// Bottom Left -	3
    };
	...
	if (target != null) {
	    ...
	    float targetTopY = target.getCollisionBounds().y + target.getCollisionBounds().height - iconHeight + MARGIN;
	    float targetBottomY = target.getCollisionBounds().y - MARGIN;
	    float targetLeftX = target.getCollisionBounds().x - MARGIN;
	    float targetRightX = target.getCollisionBounds().x + target.getCollisionBounds().width + MARGIN - iconWidth;
	    ...
	    Vector2 center = new Vector2(MainCamera.getInstance().getCenterX(), MainCamera.getInstance().getCenterY());
	...
	shapeRenderer.setColor(Color.CYAN.r, Color.CYAN.g, Color.CYAN.b, fillAlpha);
