...
public class MapForeground extends Entity {
	...
	setImages("map/foregrounds/fgLowerLeft.png", "map/foregrounds/fgLowerRight.png");
	...
	shapeRenderer.begin(ShapeType.Filled);			// Draw a stupid rectangle to fix bg bleeding
	shapeRenderer.setColor(Color.BLACK);
	shapeRenderer.rect(rightX + imageTopRight.getWidth(), -50, 100, MainCamera.getInstance().getHeight() + 50);
	shapeRenderer.end();
	...
	imageTopLeft = imageMiddleLeft = imageBottomLeft = GameManager.getInstance().assetManager.get(imageLeftDir, Texture.class);
	imageTopRight = imageMiddleRight = imageBottomRight = GameManager.getInstance().assetManager.get(imageRightDir, Texture.class);
    }
    
    public float getLeftX() 	{ return this.leftX; }
    public float getRightX() 	{ return this.rightX; }
