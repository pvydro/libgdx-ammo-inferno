...
public class PlatformGenerator extends Entity {
	...
	for (Platform p : platforms) {
	    ...
	    if (p.getY() < -100) {		// Delete platform when off the bottom of the screen
	...
	if (platforms.size() != 0) {
		...
		createNewPlatform();		// Create a new platform if the last one travelled enough
	    }
	} else {
	    createNewPlatform();
	}
	...
	if (platforms.size() == 0) {			// Check if it's the first platform
		...
		newPlatform.setX(MainCamera.getInstance().getCenterX() - (newPlatform.getWidth() / 2));
		newPlatform.setY(MainCamera.getInstance().getHeight() + 100);
		
		newPlatform.addRandomEnemy();
	    } else if ((totalPlatforms % 2) == 0) {	// Otherwise, check if the current platform is odd. If so, go on the left side
		newPlatform.setSide(0);
		newPlatform.setX(MapForeground.getInstance().getLeftX());
		newPlatform.setY(MainCamera.getInstance().getHeight() + 100);
	...
	this.platformImage = GameManager.getInstance().assetManager.get(imageDir, Texture.class);
    }
    
    public ArrayList<Platform> getPlatforms() 	{ return this.platforms; }
    public float getFirstPlatformY()		{ return this.platforms.get(0).getY(); }
    public float getSpeed()			{ return this.platformSpeed; }
