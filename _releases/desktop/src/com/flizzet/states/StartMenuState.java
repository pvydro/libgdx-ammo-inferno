...
public class StartMenuState extends GameState {
	...
	/* Check for a daily reward
	 * Otherwise if the player has played more than 4 times, ask them to rate for $$ */
	if (DailyReward.getInstance().getChecker().checkForReward(CurrentSettings.getInstance().lastPlayedDay)) {
	...
	CurrentSettings.getInstance().lastPlayedDay = DailyReward.getCurrentDay();
