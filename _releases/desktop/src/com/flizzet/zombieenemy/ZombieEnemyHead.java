...
class ZombieEnemyHead extends Entity {
	...
	image = GameManager.getInstance().assetManager.get("enemies/zombie/head/headDefault.png", Texture.class);
	...
	if (zombie.getState() != ZombieState.DEAD) {
		...
		GameManager.getInstance().particleFunctions.addBloodParticle(getCenterX(), getCenterY(), CurrentSettings.getInstance().headSplatterAmount);
	    }
	}
