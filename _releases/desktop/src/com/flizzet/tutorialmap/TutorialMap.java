...
public class TutorialMap extends Entity {
    ...
    public TutorialMap() { init(); }
    
    /** Initiates all objects */
    private void init() {
	platform.setSide(2);
	platform.setImage(GameManager.getInstance().assetManager.get("tutorial/platform.png", Texture.class));
	platform.setX(MainCamera.getInstance().getCenterX() - (platform.getWidth() / 2));
	platform.setY(150);
    }
