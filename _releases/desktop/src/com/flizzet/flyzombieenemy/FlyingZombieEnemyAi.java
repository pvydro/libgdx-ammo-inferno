...
public class FlyingZombieEnemyAi extends EnemyAi {
	...
	if (enemy.getState() == FlyingEnemyState.CHASING_PLAYER) {
	    float distanceX = 10 * ((player.getCenterX() - enemy.getX()) / 2) * delta;
	    float distanceY = 10 * ((player.getCenterY() - enemy.getY()) / 2) * delta;
	    if (distanceX != 0 && distanceY != 0) {
		float factor = (float) (speed / Math.sqrt(distanceX * distanceX + distanceY * distanceY));
		distanceX *= factor;
		distanceY *= factor;
	    }
