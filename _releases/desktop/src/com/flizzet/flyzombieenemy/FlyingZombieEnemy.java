...
/**
 * Concrete flying zombie enemy class.
 *
 * @author Pedro Dutra (2017)
 * @version 1.0
 */
public class FlyingZombieEnemy extends FlyingEnemy {
    
    private FlyingEnemyState state = FlyingEnemyState.CHASING_PLAYER;
    private FlyingZombieEnemyCollision collision = new FlyingZombieEnemyCollision(this);
    ...
    public FlyingZombieEnemy() {
	
	headImage = GameManager.getInstance().assetManager.get("enemies/flyingZombie/head.png", Texture.class);
	this.setHealthBar(new FlyingZombieHealthBar(this));
	this.setAi(new FlyingZombieEnemyAi(this));
	adjustBoundsToImage(headImage);
	
	/* Building animations */
	Texture wingSheetLeft = GameManager.getInstance().assetManager.get("enemies/flyingZombie/zombieWingSheetLeft.png", Texture.class);
	Texture wingSheetRight = GameManager.getInstance().assetManager.get("enemies/flyingZombie/zombieWingSheetRight.png", Texture.class);
	...
	rightWingX = bounds.x + bounds.width + 3 - rightWingAnim.getKeyFrame(0).getRegionWidth();
	wingY = bounds.y - 3;
	...
	case 0:
	    ...
	    batch.draw(headImage, bounds.x, bounds.y, bounds.width, bounds.height);
	...
	if (CurrentSettings.getInstance().showCollisions) {
	    ...
	    shapeRenderer.rect(getCollisionBounds().x, getCollisionBounds().y, getCollisionBounds().width,
		    getCollisionBounds().height);
	    shapeRenderer.end();
	    batch.begin();
	}
