...
public class StageManager implements Updatable {
	...
	if (Player.getInstance().getScore().getPlatforms() >= currentPlatformLimit) {
	    nextStage();
	...
	if (currentStage < 11) {
	    ...
	    } else if (MainEnemySpawner.getInstance().isFlyingZombiesEnabled()) {
	...
	if (Upgrades.getInstance().isEquipped(EarlyBirdUpgrade.class)) {
	    if (currentStage != 2) {
		GameManager.getInstance().particleFunctions.addNextStageParticle();
	    }
	} else {
	    if (currentStage != 1) {
		GameManager.getInstance().particleFunctions.addNextStageParticle();
	...
	GameNotification stageNumNoti = new GameNotification("stage " + String.valueOf(currentStage));
