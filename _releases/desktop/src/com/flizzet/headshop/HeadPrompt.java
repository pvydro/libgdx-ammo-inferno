...
class HeadPrompt extends GuiComponent {
	...
	iconX = bounds.x + MainCamera.getInstance().getCenterX() - (iconWidth / 2) + view.getXChange();
	iconY = bounds.y + MainCamera.getInstance().getCenterY();
	
	/* Setting name position */
	nameX = bounds.x + MainCamera.getInstance().getCenterX() - (nameLayout.width / 2);
	nameY = iconY - 5;
    ...
    public PromptBuyButton getBuyButton()	{ return this.buyButton; }
