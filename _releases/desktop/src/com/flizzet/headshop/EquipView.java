...
public class EquipView extends GuiComponent {
     ...
     * @param view EquipView is dependent on a IconView for positioning */
    public EquipView(IconView view) {
	super(0, 0);
	this.view = view;
	currentHeadIndex = Heads.getInstance().getHeads().indexOf(Heads.getInstance().getEquippedHead());
    }
	...
	headX = bounds.x + MainCamera.getInstance().getCenterX() - (headWidth / 2);
	...
	/* Position text */
	headerX = bounds.x + MainCamera.getInstance().getCenterX() - (layout.width / 2);
	...
	batch.draw(currentHead.getIconImage(), headX, headY, headWidth, headHeight);
	...
	/* Equip new head */
	Heads.getInstance().equip(Heads.getInstance().getHeads().get(currentHeadIndex).getClass());
    }
