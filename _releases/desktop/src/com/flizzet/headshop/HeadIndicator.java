...
class HeadIndicator extends GuiComponent {
	...
	super(0, 0);
	this.view = view;
	this.image = GameManager.getInstance().assetManager.get("gui/headShop/indicator.png", Texture.class);
    }

    @Override
    public void update(float delta) {
	
	HeadIcon icon = view.getHeadIcon(Heads.getInstance().getHeads().indexOf(Heads.getInstance().getEquippedHead()));
