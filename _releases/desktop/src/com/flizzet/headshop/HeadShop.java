...
public class HeadShop extends GuiComponent {
	...
	headerX = menuBuilder.getIconView().getX() + MainCamera.getInstance().getCenterX() + (totalWidth / 2) - headerLayout.width;
	headerY = MainCamera.getInstance().getHeight() - MARGIN;
	
	/* Set back button position */
	backButton.setX(menuBuilder.getIconView().getX() + MainCamera.getInstance().getCenterX() - (totalWidth / 2));
