...
public class ReviveMenu extends GuiComponent {
	...
	skullImage = GameManager.getInstance().assetManager.get("gui/reviveMenu/skull.png");
	...
	/* Position skull */
	skullX = MainCamera.getInstance().getCenterX() - ((skullImage.getWidth()) / 2);
	skullY = MainCamera.getInstance().getCenterY() + 50;
	
	float newXJitter = random.nextBoolean() ? random.nextInt(jitterPositionAmt) : -random.nextInt(jitterPositionAmt);
	jitterX += (newXJitter - jitterX) / 4;
	float newYJitter = random.nextBoolean() ? random.nextInt(jitterPositionAmt) : -random.nextInt(jitterPositionAmt);
	...
	textX = MainCamera.getInstance().getCenterX() - (reviveLayout.width / 2);
	...
	if (CurrentSettings.getInstance().adsPlayable) {
	    reviveButton.setX(MainCamera.getInstance().getCenterX() - reviveButton.getWidth() - 2);
	} else {
	    reviveButton.setX(MainCamera.getInstance().getCenterX() - (reviveButton.getWidth() / 2));
	}
	reviveButton.setY(MainCamera.getInstance().getCenterY() - (reviveButton.getHeight() / 2));
	reviveButton.update(delta);
	/* Position play video button */
	if (Player.getInstance().getScore().getTotalBills() < 15) {
	    videoButton.setX(MainCamera.getInstance().getCenterX() - (videoButton.getWidth() / 2));
	} else {
	    videoButton.setX(MainCamera.getInstance().getCenterX() + 2);
	}
	videoButton.setY(MainCamera.getInstance().getCenterY() - (videoButton.getHeight() / 2));
	...
	/* Draw revive button */
	if (Player.getInstance().getScore().getTotalBills() >= 15) reviveButton.render(batch);
	if (CurrentSettings.getInstance().adsPlayable) videoButton.render(batch);
    }
    ...
    public ReviveButton getReviveButton()	{ return this.reviveButton; }
