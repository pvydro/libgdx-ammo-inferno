...
public class ResumeButton extends ButtonComponent {
	...
	this.setImage(GameManager.getInstance().assetManager.get("gui/pauseMenu/resumeButton.png", Texture.class));
	this.setImageHovered(GameManager.getInstance().assetManager.get("gui/pauseMenu/resumeButton.png", Texture.class));
	this.setImagePushed(GameManager.getInstance().assetManager.get("gui/pauseMenu/resumeButtonPushed.png", Texture.class));
    }
	...
	if (isPushed()) {
	    batch.draw(shineImage, bounds.x + 2 + shakeXOffset, subtextY + 1 + 2 + shakeYOffset, bounds.width - 4,
		    bounds.height - 4);
	} else {
