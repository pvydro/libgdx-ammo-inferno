...
public class InputController implements GestureListener {
	...
	for (GuiComponent b : InGameGui.getInstance().getButtons()) {
	    if (b.getBounds().contains(MainCamera.getInstance().getMousePos().x, MainCamera.getInstance().getMousePos().y)) {
		return false;
	    }
	}
	/* Double tap */
	if (count == 2) {
		...
		&& GameManager.getInstance().stateManager.getCurrentState() == GameManager.getInstance().stateId_play) {
		
		TargetButton.getInstance().triggered();
		
	    } else if (PauseManager.getInstance().getState() == GameState.SELECT) {
		TargetButton.getInstance().triggered();
	    }
	    count = 0;
	}
