...
public class CurrentUpgradesBar extends GuiComponent {
	...
	private Upgrade[] currentlyEquipped = Upgrades.getInstance()
			.getEquippedUpgrades();
	...
	public CurrentUpgradesBar() {
		...
		this.barImage = GameManager.getInstance().assetManager
				.get("gui/upgradeShop/currentUpgrades.png");
		this.indicatorImage = GameManager.getInstance().assetManager
				.get("gui/upgradeShop/currentSlotIndicator.png");
		adjustBoundsToImage();
	...
	public void update(float delta) {
		...
		if (Gdx.input.isTouched()) {
			...
			if (mouseX < getX() + getWidth()
					&& MainCamera.getInstance().getMousePos().y < this.getY()
							+ this.getHeight()
