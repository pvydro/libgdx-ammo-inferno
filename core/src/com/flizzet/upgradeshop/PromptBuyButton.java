...
class PromptBuyButton extends ButtonComponent {
	...
	private Texture shineImage = GameManager.getInstance().assetManager
			.get("gui/upgradeShop/prompt/buttonShine.png");
			...
			case 0:
				if (CurrentSettings.getInstance().brokeBoi) {
					...
					GameNotification notification = new GameNotification(
							"bought " + upgrade.getName());
					GameManager.getInstance().guiContainer
							.addToGui(notification);
				} else if (Player.getInstance().getScore()
						.getTotalCoins() > Upgrades.getInstance()
								.getUpgrade(upgrade.getClass()).getPrice()) {
					Sounds.play(Sounds.getInstance().buySound, 1.0f);
	...
	public void setState(int newState) {
		...
		switch (newState) {
			...
			case 1:
				this.setImage(GameManager.getInstance().assetManager
						.get("gui/prompt/equipButton.png", Texture.class));
				this.setImageHovered(GameManager.getInstance().assetManager
						.get("gui/prompt/equipButton.png", Texture.class));
				this.setImagePushed(GameManager.getInstance().assetManager.get(
						"gui/prompt/equipButtonPushed.png", Texture.class));
				break;
			case 2:
				this.setImage(GameManager.getInstance().assetManager
						.get("gui/prompt/equippedButton.png", Texture.class));
				this.setImageHovered(GameManager.getInstance().assetManager
						.get("gui/prompt/equippedButton.png", Texture.class));
				this.setImagePushed(GameManager.getInstance().assetManager.get(
						"gui/prompt/equippedButtonPushed.png", Texture.class));
				break;
		}
	}
