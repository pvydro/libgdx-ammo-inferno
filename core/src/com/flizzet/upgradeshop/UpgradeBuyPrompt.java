...
public class UpgradeBuyPrompt extends GuiComponent {
	...
	public UpgradeBuyPrompt(UpgradeScrollView view) {
		super(0, 0);
		this.view = view;
		this.icon = GameManager.getInstance().assetManager
				.get("upgrades/icons/stickySoles.png");
	...
	public void update(float delta) {
		...
		iconX = bounds.x + MainCamera.getInstance().getCenterX()
				- ((icon.getWidth() * ICON_MULTIPLIER) / 2);
		...
		headerY = iconY + headerLayout.height
				+ (icon.getWidth() * ICON_MULTIPLIER) + 5;

		/* Setting description position */
		descLayout.setText(descFont, description, Color.WHITE,
				MainCamera.getInstance().getWidth(), Align.center, true);
		descX = bounds.x + MainCamera.getInstance().getCenterX()
				- (MainCamera.getInstance().getWidth() / 2);
		descY = iconY - 5;
	...
	public void render(SpriteBatch batch) {
		...
		batch.draw(icon, iconX, iconY, icon.getWidth() * ICON_MULTIPLIER,
				icon.getHeight() * ICON_MULTIPLIER);
