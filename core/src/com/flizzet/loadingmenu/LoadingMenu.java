...
public class LoadingMenu extends GuiComponent {
	...
	public void update(float delta) {
		...
		/* Loading textures Single call */
		if (!loaded) {
			/* Finish loading all textures */
			GameManager.getInstance().assetManager
					.finishLoadingAsset("gui/loadingMenu/background.png");
			GameManager.getInstance().assetManager
					.finishLoadingAsset("gui/loadingMenu/barFill.png");
			GameManager.getInstance().assetManager
					.finishLoadingAsset("gui/loadingMenu/barOutline.png");
			GameManager.getInstance().assetManager
					.finishLoadingAsset("lights/loadingBarLight.png");
			GameManager.getInstance().assetManager
					.finishLoadingAsset("gui/constant/menuBgOverlayOpaque.png");
			GameManager.getInstance().assetManager
					.finishLoadingAsset("gui/loadingMenu/logo.png");
			/* Set all textures */
			background = GameManager.getInstance().assetManager
					.get("gui/loadingMenu/background.png", Texture.class);
			barFill = GameManager.getInstance().assetManager
					.get("gui/loadingMenu/barFill.png", Texture.class);
			barOutline = GameManager.getInstance().assetManager
					.get("gui/loadingMenu/barOutline.png", Texture.class);
			logo = GameManager.getInstance().assetManager
					.get("gui/loadingMenu/logo.png", Texture.class);
		...
		/* Position background */
		backgroundX = MainCamera.getInstance().getCenterX()
				- (background.getWidth() / 2);
		backgroundY = MainCamera.getInstance().getCenterY()
				- (background.getHeight() / 2);

		/* Position header */
		headerX = MainCamera.getInstance().getCenterX()
				- (headerLayout.width / 2);
		...
		loadingBarX = MainCamera.getInstance().getCenterX()
				- (barOutline.getWidth() / 2);
		loadingBarY = MARGIN;

		/* Positioning amount text */
		float newWidth = barFill.getWidth()
				* GameManager.getInstance().assetManager.getProgress();
		amount = String.valueOf(
				(int) (GameManager.getInstance().assetManager.getProgress()
						* 100))
				+ "%";
		amountLayout.setText(amountFont, amount);
		amountX = loadingBarX + newWidth - amountLayout.width - 2;
		amountY = loadingBarY + (barOutline.getHeight() / 2)
				+ (amountLayout.height / 2);

		/* Positioning loading bar light */
		light.setWidth(newWidth + 200);
		light.setX(loadingBarX + (newWidth / 2) - (light.getWidth() / 2));
		light.setY(loadingBarY + (barOutline.getHeight() / 2)
				- (light.getHeight() / 2));
	}
	...
	public void render(SpriteBatch batch) {
		...
		float newWidth = barFill.getWidth()
				* GameManager.getInstance().assetManager.getProgress();
		batch.draw(barFill, loadingBarX + 4, loadingBarY + 4, newWidth,
				barFill.getHeight());
