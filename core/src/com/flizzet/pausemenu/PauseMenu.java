...
public class PauseMenu extends GuiComponent {
	...
	private GlyphLayout platformsLayout = new GlyphLayout(platformsFont,
			platforms);
	...
	private PauseMenu() {
		...
		/*
		 * Set platforms position SET in constructor for the menuBuilder to use
		 * the margin
		 */
		platformsX = MainCamera.getInstance().getCenterX()
				- (platformsLayout.width / 2);
	...
	public void update(float delta) {
		...
		/* Set score position */
		scoreX = MainCamera.getInstance().getCenterX()
				- (scoreLayout.width / 2);
