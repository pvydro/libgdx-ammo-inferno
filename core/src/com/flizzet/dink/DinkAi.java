...
class DinkAi implements Updatable {
	...
	public void update(float delta) {
		...
		/*
		 * Follow player along with new position Or follow enemy
		 */
		float newX = 0;
		float newY = 0;
		...
		if (dink.getState() == DinkState.FOLLOWING) {
			...
			for (Platform p : PlatformGenerator.getInstance().getPlatforms()) {
				if (p.getZombieEnemy() != null && !p.getZombieEnemy().isDead()
						&& p.getZombieEnemy().getY() < 400) {
					targetSelected = true;
					newX = p.getZombieEnemy().getCenterX()
							- (dink.getWidth() / 2);
					newY = p.getZombieEnemy().getCenterY()
							- (dink.getHeight() / 2);
				}
			}
			if (!targetSelected) {
	...
	private void findNewPosition() {
		targetX = random.nextBoolean()
				? random.nextInt(20)
				: -random.nextInt(20);
		targetY = random.nextBoolean()
				? random.nextInt(20)
				: -random.nextInt(10);
	}
