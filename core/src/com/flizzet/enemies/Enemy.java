...
public abstract class Enemy extends Entity {
	...
	protected EnemyIndicator indicator = new EnemyIndicator(this);
	...
	public void die() {
		/* Remove indicator */
		indicator.setRemovalCooldown(0);
		
		/* Increase weapon total kills */
		Player.getInstance().getWeapon().setTotalKills(
				Player.getInstance().getWeapon().getTotalKills() + 1);

		/* Add a death particle to the screen */
		if (CurrentSettings.getInstance().enemyDeath) {
			GameManager.getInstance().particleFunctions
					.addEnemyDeathParticle(getCenterX(), getCenterY());
			GameManager.getInstance().particleFunctions
					.addEnemyDeathSlashParticle(getCenterX(), getCenterY());
		}
	}
	...
	public void setCollisionWidth(float newWidth) {
		this.collisionBounds.setWidth(newWidth);
	}
	public void setCollisionHeight(float newHeight) {
		this.collisionBounds.setHeight(newHeight);
	}
	public void setCollisionX(float newX) {
		this.collisionBounds.setX(newX);
	}
	public void setCollisionY(float newY) {
		this.collisionBounds.setY(newY);
	}
