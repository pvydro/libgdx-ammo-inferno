...
public class FlyingEnemyAi extends EnemyAi {
	...
	public void update(float delta) {
		...
		/*
		 * Matching target position to side of platforms or Moving target
		 * position to player position
		 */
		if (state != FlyingEnemyState.CHASING_PLAYER
				&& enemy.getY() < Player.getInstance().getCenterY()) {
			/* Move enemy up platforms */
			state = FlyingEnemyState.CHASING_PLATFORM; // Set state to moving
														// towards platforms
			if (enemy.getY() < targetPlatform.getY() - 65) {
				enemy.setY(enemy.getY() + 2); // Move the enemy up to the
												// platform
			} else {
			...
			if (targetPlatform.getSide() == 2
					|| targetPlatform.getSide() == 1) { // Right
				targetX = targetPlatform.getX() + targetPlatform.getWidth()
						- 20;
			} else { // Left
			...
			enemy.setY(
					enemy.getY() + PlatformGenerator.getInstance().getSpeed());
		} else {
			state = FlyingEnemyState.CHASING_PLAYER; // Set state to moving
														// towards player
	...
	private void targetNextPlatform() {
		...
		if (totalPlatforms - totalPlatformsRemoved <= platforms.size() - 1) {
			targetPlatform = platforms
					.get(totalPlatforms - totalPlatformsRemoved);
		} else {
			enemy.die();
		}
	}
	...
	private void remove() {
		if (enemy instanceof FlyingEyeEnemy) {
			FlyingEyeEnemySpawner.getInstance()
					.removeFromEyes((FlyingEyeEnemy) enemy);
		}
	...
	public void setTotalPlatforms(int newTotalPlatforms) {
		this.totalPlatforms = newTotalPlatforms;
	}
	public void platformRemoved() {
		this.totalPlatformsRemoved++;
	}
