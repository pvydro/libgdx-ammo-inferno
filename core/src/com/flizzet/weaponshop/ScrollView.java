...
public class ScrollView extends GuiComponent {
	...
	private void buildScrollWindow() {
		...
		currentSelection = Weapons.getInstance().getWeapons()
				.indexOf(Weapons.getInstance().getEquippedWeapon());
	}
	...
	public void update(float delta) {
		...
		if (pushed) {
			...
			bounds.x += ((-views.get(currentSelection).getX()
					- (views.get(currentSelection).getImageWidth() / 2)
					+ MainCamera.getInstance().getCenterX() - bounds.x) / 5);
		}
		...
		int columns = 0;
		for (WeaponView w : views) {
			w.setX(MainCamera.getInstance().getCenterX()
					- (w.getImageWidth() / 2) + (MARGIN * columns));
		...
		light.update(delta);
		light.setX(
				MainCamera.getInstance().getCenterX() - (light.getWidth() / 2));
		light.setY(MainCamera.getInstance().getCenterY()
				- (light.getHeight() / 2));
	...
	public WeaponView getCurrentView() {
		return this.views.get(currentSelection);
	}
	public ArrayList<WeaponView> getViews() {
		return this.views;
	}
	public WeaponStand getStand() {
		return this.stand;
	}
