...
public class PlatformLamp extends Entity {
	...
	private int particleCooldown = CurrentSettings
			.getInstance().platformLightParticleAmount;
	...
	public PlatformLamp(Platform platform) {
		...
		this.image = GameManager.getInstance().assetManager
				.get("items/platformLight.png", Texture.class);
	...
	public void update(float delta) {
		...
		if (platform.getSide() == 0)
			this.setX(platform.getX() + 11 + xOffset);
		else
		...
		if (CurrentSettings.getInstance().platformLightParticles) {
			...
			if (particleCooldown <= 0) {
				float xOffset = random.nextBoolean()
						? random.nextFloat()
						: -random.nextFloat();
				GameManager.getInstance().particleFunctions
						.addPlatformLampParticle(
								this.getCenterX() + xOffset - .5f,
								bounds.y + 9);
				particleCooldown = CurrentSettings
						.getInstance().platformLightParticleAmount;
			}
		}
