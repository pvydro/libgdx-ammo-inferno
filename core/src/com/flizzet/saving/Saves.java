...
public class Saves {
	...
	public static Preferences prefs = Gdx.app.getPreferences("svflizzetscj003");// svflizzetscj002
	...
	public void load() {
		...
		/*
		 * Assign default variables if it's the first time playing Otherwise,
		 * load everything
		 */
		CurrentSettings.getInstance().playedBefore = prefs
				.getBoolean("playedBefore");
	...
	public void loadEconomy() {
		...
		Player.getInstance().getScore()
				.setPlatforms(prefs.getInteger("platforms"));
		Player.getInstance().getScore()
				.setHighScore(prefs.getInteger("highscore"));
	}
	...
	public void loadUpgrades() {
		for (Upgrade u : Upgrades.getInstance().getUpgrades()) {
			...
			if (prefs.getBoolean(u.getName() + " equipped")) {
				...
				Upgrades.getInstance().setCurrentSlot(
						Upgrades.getInstance().getCurrentSlot() + 1);
			}
		}
	}
	...
	public void loadControls() {
		CurrentSettings.getInstance().controls = ControlType.values()[prefs
				.getInteger("controlType")];
	}

	/** Loads effect settings from the save file */
	public void loadEffects() {
		CurrentSettings.getInstance().collideParticles = prefs
				.getBoolean("collideParticles");
		CurrentSettings.getInstance().softLight = prefs.getBoolean("softLight");
		CurrentSettings.getInstance().screenFlash = prefs
				.getBoolean("screenFlash");
		CurrentSettings.getInstance().screenShake = prefs
				.getBoolean("screenShake");
		CurrentSettings.getInstance().bugs = prefs.getBoolean("bugs");
	}

	/** Loads particle settings from the save file */
	public void loadParticles() {
		CurrentSettings.getInstance().blood = prefs
				.getBoolean("bloodParticles");
		CurrentSettings.getInstance().textParticles = prefs
				.getBoolean("textParticles");
		CurrentSettings.getInstance().groundParticles = prefs
				.getBoolean("walkingParticles");
		CurrentSettings.getInstance().bulletHits = prefs
				.getBoolean("bulletHitParticles");
		CurrentSettings.getInstance().platformLightParticles = prefs
				.getBoolean("lightParticles");
		CurrentSettings.getInstance().lavaParticles = prefs
				.getBoolean("lavaParticles");
		CurrentSettings.getInstance().enemyHits = prefs
				.getBoolean("enemyHitParticles");
		CurrentSettings.getInstance().enemyDeath = prefs
				.getBoolean("enemyDeathParticles");
		CurrentSettings.getInstance().playerLand = prefs
				.getBoolean("landParticles");
	}
	...
	public void loadGeneral() {
		...
		CurrentSettings.getInstance().doubleTapSelect = prefs
				.getBoolean("doubleTap");
	}

	/** Loads total days in a row data from the save file */
	public void loadDays() {
		CurrentSettings.getInstance().lastPlayedDay = prefs
				.getInteger("lastPlayedDay");
	...
	public void loadPlays() {
		CurrentSettings.getInstance().totalPlays = prefs
				.getInteger("totalPlays");
		CurrentSettings.getInstance().askedToRate = prefs
				.getBoolean("askedToRate");
	}

	/** Saves the economy elements to the save file */
	public void saveEconomy() {
		prefs.putInteger("coins",
				Player.getInstance().getScore().getTotalCoins());
		prefs.putInteger("bills",
				Player.getInstance().getScore().getTotalBills());
		prefs.putInteger("platforms",
				Player.getInstance().getScore().getTotalPlatforms());
		prefs.putInteger("highscore",
				Player.getInstance().getScore().getHighScore());
	...
	public void saveDays() {
		prefs.putInteger("lastPlayedDay",
				CurrentSettings.getInstance().lastPlayedDay);
	...
	public void savePlays() {
		prefs.putInteger("totalPlays",
				CurrentSettings.getInstance().totalPlays);
		prefs.putBoolean("askedToRate",
				CurrentSettings.getInstance().askedToRate);
		prefs.flush();
	}

	public Preferences get(Preferences prefs) {
		return prefs;
	}
