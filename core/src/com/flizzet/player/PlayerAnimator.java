...
class PlayerAnimator implements Renderable, Updatable {
	...
	public PlayerAnimator(Player player) {
		...
		playerIdleImage = GameManager.getInstance().assetManager
				.get("player/body/bodyIdle.png", Texture.class);
		playerWalkingSheet = GameManager.getInstance().assetManager
				.get("player/body/bodyWalking.png", Texture.class);
		playerJumpingSheet = GameManager.getInstance().assetManager
				.get("player/body/bodyJumping.png", Texture.class);

		walkingAnim = AnimationUtils.newAnimation(playerWalkingSheet, 12, 1,
				40);
	...
	public void render(SpriteBatch batch) {
		...
		if (player.getState() == PlayerState.JUMPING) {
			playerCurrentFrame = jumpingAnim.getKeyFrame(jumpingStateTime,
					false);
		} else if (player.getState() == PlayerState.WALKING) {
			playerCurrentFrame = walkingAnim.getKeyFrame(walkingStateTime,
					false);
		} else {
		...
		if (CurrentSettings.getInstance().showCollisions) {
			...
			shapeRenderer.rect(player.getCollisionBounds().x,
					player.getCollisionBounds().y,
					player.getCollisionBounds().width,
					player.getCollisionBounds().height);
			shapeRenderer.end();
			batch.begin();
		}
	...
	public int getWalkFrameIndex() {
		return walkingAnim.getKeyFrameIndex(walkingStateTime);
	}
}
