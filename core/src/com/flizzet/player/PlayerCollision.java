...
class PlayerCollision implements Updatable {
	...
	public void update(float delta) {
		...
		/* Lava collision */
		if (player.getCollisionBounds()
				.overlaps(Lava.getInstance().getBounds())) {
			if (player.getState() != PlayerState.DEAD) {
				if (player.getState() != PlayerState.DYING && Upgrades
						.getInstance().isEquipped(LavaBootsUpgrade.class)) {
		...
		/* Stopping on sides */
		if (player.getCollisionBounds().x < MapForeground.getInstance()
				.getLeftX() + 11) {
			player.getController().bounceRight(3);
		}
		if (player.getCollisionBounds().x
				+ player.getCollisionBounds().getWidth() > MapForeground
						.getInstance().getRightX()) {
			player.getController().bounceLeft(3);
		}

		/* Stopping on "ceiling" */
		if (player.getCollisionBounds().y > MainCamera.getInstance()
				.getHeight()) {
			if (platforms.get(0).getY() < MainCamera.getInstance().getHeight()
	...
	private void stopPlayer(Platform platform) {
		if (player.getController().isJumping()) {
			...
			if (player.getY() > platform.getY() + platform.getHeight() - 1
					&& CurrentSettings.getInstance().playerLand) {
				GameManager.getInstance().particleFunctions
						.addPlayerLandParticle(player.getCenterX(),
								platform.getY() + platform.getHeight());
				Sounds.play(Sounds.getInstance().playerLandSound, 1.0f,
