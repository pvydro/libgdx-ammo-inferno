		...
		if (this.getY() < 0 && this.getState() != PlayerState.DEAD) {
			...
			reviveMenu = new ReviveMenu() {
				...
				public void triggered() {
					...
					TransitionComponent transition = new TransitionComponent(0,
							0) {
						@Override
						public void triggered() {
							...
							GameManager.getInstance().guiContainer
									.addToGui(DeathMenu.getInstance());
						}
					};
					...
					/*
					 * Show an interstitial ad 1 in 3 times if the player has
					 * played before
					 */
			...
			} else if (score.getTotalBills() < 15
					&& !CurrentSettings.getInstance().adsPlayable) {
				reviveMenu.triggered();
			} else {
