...
public class PlayerHead extends Entity {
	...
	private int currentCrown = 0; // 0 = No crown; 1 = Wood; 2 = Silver; 3 =
									// Gold
	...
	public PlayerHead(Player player) {
		...
		image = GameManager.getInstance().assetManager
				.get("player/head/miner.png", Texture.class);
		adjustBoundsToImage(image);

		crowns = new Texture[]{
				GameManager.getInstance().assetManager
						.get("player/head/crowns/wood.png"),
				GameManager.getInstance().assetManager
						.get("player/head/crowns/silver.png"),
				GameManager.getInstance().assetManager
						.get("player/head/crowns/gold.png")};
	}
	...
	public void update(float delta) {
		...
		/* Set crown positions */
		crownX = this.getX() + (this.getWidth() / 2)
				- (crowns[0].getWidth() / 2) + crownXOffset;
