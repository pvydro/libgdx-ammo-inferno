...
public class BulletCollision implements Updatable {
	...
	public void update(float delta) {
		...
		for (Platform p : platforms) {
			if (p.getZombieEnemy() != null) { // If there's an enemy on the
												// platform
		...
		for (FlyingZombieEnemy e : FlyingZombieEnemySpawner.getInstance()
				.getZombies()) {
			performCollisionWithEnemy(e);
		}
		/* Dummy collision */
		...
		for (Platform p : platforms) {
			if (bulletBounds.overlaps(p.getBounds())) {
				if (cooldown <= 0) {
					if (Upgrades.getInstance().isEquipped(
							StyrofoamDartsUpgrade.class) && hits < 1) {
						bullet.bounce();
						hits++;
					} else {
						bullet.remove();
		...
		if (bulletBounds.x < MapForeground.getInstance().getLeftX() + 11) {
			...
			if (random.nextInt(10) == 4)
				Sounds.play(Sounds.getInstance().wallHitSound, 0.4f,
		...
		if (bulletBounds.x + bulletBounds.getWidth() > MapForeground
	...
	private void performCollisionWithEnemy(Enemy enemy) {
		if (bulletBounds.overlaps(enemy.getCollisionBounds())) {
			...
			if (CurrentSettings.getInstance().enemyHits) {
				GameManager.getInstance().particleFunctions.addEnemyHitParticle(
						enemy.getCenterX(), bullet.getCenterY());
			}
		}
	}
