...
public class BackgroundComponent extends GuiComponent {
	...
	public BackgroundComponent() {
		...
		this.gradientOverlay = GameManager.getInstance().assetManager
				.get("gui/constant/menuBgGradient.png");
		this.zombieFiller = GameManager.getInstance().assetManager
				.get("gui/constant/zombieFiller.png");
		this.buildHeads();
	}
	...
	public void render(SpriteBatch batch) {
		...
		shapeRenderer.rect(0, 0, MainCamera.getInstance().getWidth(),
				MainCamera.getInstance().getHeight());
		shapeRenderer.end();
		batch.begin();
		...
		shapeRenderer.rect(-200, -200, 200,
				MainCamera.getInstance().getHeight() + 400);
		shapeRenderer.rect(MainCamera.getInstance().getWidth(), -200, 200,
				MainCamera.getInstance().getHeight() + 400);
		shapeRenderer.end();
		batch.begin();
	...
	private void buildHeads() {
		...
		for (int i = 0; i < totalHeads; i++) {
			...
			/* Create head */
			BackgroundHead newHead = new BackgroundHead(currentRow + 1, alpha,
					this);
			/* Set head position */
	...
	private void newParticle() {
		...
		p.setX(random.nextInt(
				(int) (MainCamera.getInstance().getWidth() - p.getWidth())
						+ 200)
				- 100);
	...
	public float getScreenFlashAmount() {
		return this.screenFlashAmount;
	}
