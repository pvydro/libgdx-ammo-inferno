...
public class FireParticle extends Particle {
	...
	public FireParticle() {
		...
		color = new Color(random.nextFloat() * 3, random.nextFloat() * 3,
				random.nextFloat() / 5, 1.0f);
	...
	public void update(float delta) {
		...
		this.setY(this.getY() + (yVel * delta)
	...
	public void render(SpriteBatch batch, ShapeRenderer shapeRenderer) {
		if (draw) {
			...
			shapeRenderer.rect(bounds.x + xChange, bounds.y, bounds.width,
					bounds.height);
			shapeRenderer.end();
			batch.begin();
		}
