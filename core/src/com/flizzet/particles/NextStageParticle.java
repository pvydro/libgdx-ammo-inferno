...
public class NextStageParticle extends Particle {
	...
	public NextStageParticle() {
		...
		this.textImage = GameManager.getInstance().assetManager
				.get("gui/inGame/nextStageNoti.png");
		this.bgImage = GameManager.getInstance().assetManager
				.get("gui/inGame/nextStageBg.png");

		/* Set initial positions */
		textX = 1000;
		textY = MainCamera.getInstance().getCenterY()
	...
	public void update(float delta) {
		...
		/* Ease */
		float centerTextX = MainCamera.getInstance().getCenterX()
				- (textImage.getWidth() / 2);
		float centerBgX = MainCamera.getInstance().getCenterX()
				- (bgImage.getWidth() / 2);
