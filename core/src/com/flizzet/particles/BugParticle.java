...
public class BugParticle extends Particle {
	...
	public void update(float delta) {
		...
		/*
		 * Move to target if not dead Otherwise move with vels
		 */
		if (state != 2) {
			/* Move around move around */
			bounds.x += 10 * ((((player.getCenterX() + (player.getWidth() / 2))
					+ targetX) - bounds.x) / speed) * delta;
			bounds.y += 10 * ((((player.getCenterY() + (player.getHeight() / 2))
					+ targetY) - bounds.y) / speed) * delta;
		} else { // Dead
