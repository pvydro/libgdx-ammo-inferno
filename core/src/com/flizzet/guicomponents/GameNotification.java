...
public class GameNotification extends GuiComponent {
	...
	public GameNotification(String message) {
		...
		this.image = GameManager.getInstance().assetManager
				.get("gui/constant/notification.png");
	...
	public void update(float delta) {
		...
		if (state == 0) {
			bounds.y += ((MainCamera.getInstance().getHeight()
					- this.getHeight()) - bounds.y) / 5;
		} else {
			bounds.y += ((MainCamera.getInstance().getHeight() + 5) - bounds.y)
					/ 5;
		}

		/* Text positioning */
		layout.setText(font, message, Color.WHITE, this.getWidth(),
				Align.center, true);
		...
		/* Removal */
		if (state == 1
				&& this.getY() > MainCamera.getInstance().getHeight() + 1) {
