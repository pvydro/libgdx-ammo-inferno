...
public class CheckboxComponent extends GuiComponent {
	...
	public CheckboxComponent(String title, String info, boolean param) {
		...
		this.imageOn = GameManager.getInstance().assetManager
				.get("gui/settingsMenu/constant/checkboxOn.png");
		this.imageOff = GameManager.getInstance().assetManager
				.get("gui/settingsMenu/constant/checkboxOff.png");
	...
	public void update(float delta) {
		...
		titleX = 20;
		titleY = this.getY() + (this.getHeight() / 2)
				+ (titleLayout.height / 2);

		/* Settings info button position */
		infoButton.setX(titleX + titleLayout.width - 12);
		infoButton.setY(titleY - (titleLayout.height / 2)
				- (infoButton.getHeight() / 2));
	...
	public void render(SpriteBatch batch) {
		...
		if (Gdx.input.isTouched()) {
			if (bounds.contains(MainCamera.getInstance().getMousePos().x,
					MainCamera.getInstance().getMousePos().y)) {
				batch.setColor(new Color(tmp.r - 0.3f, tmp.g - 0.3f,
						tmp.b - 0.3f, 1.0f));
			}
		}
		...
		if (state == CheckboxState.ON)
			batch.draw(imageOn, bounds.x, bounds.y);
		else if (state == CheckboxState.OFF)
			batch.draw(imageOff, bounds.x, bounds.y);
...
class CheckboxInfoButton extends ButtonComponent {
	...
	CheckboxInfoButton(String info) {
		...
		this.setImage(GameManager.getInstance().assetManager.get(
				"gui/settingsMenu/constant/checkboxInfoButton.png",
				Texture.class));
		...
		this.setImagePushed(GameManager.getInstance().assetManager.get(
				"gui/settingsMenu/constant/checkboxInfoButtonPushed.png",
				Texture.class));
	}
	...
	public void update(float delta) {
		/*
		 * Prevent the button from being pushable if there is already an info
