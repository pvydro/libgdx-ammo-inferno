...
public class SplashTitleComponent extends GuiComponent {
	...
	public SplashTitleComponent(float x, float y) {
		...
		titleImage = GameManager.getInstance().assetManager
				.get("gui/splashScreen/flizLuseLogo.png", Texture.class);
	...
	public void update(float delta) {
		...
		logoX = bounds.x + (bounds.width / 2) - (titleImage.getWidth() / 14); // Use
																				// /14
																				// because
																				// the
																				// picture
	...
	public void render(SpriteBatch batch) {
		...
		/* Locals */
		titleCurrentFrame = titleAnimation.getKeyFrame(stateTime, false); // Get
																			// the
																			// key
																			// frame
																			// of
