...
public class InfoPrompt extends GuiComponent {
	...
	public InfoPrompt(String message) {
		...
		backgroundImage = GameManager.getInstance().assetManager
				.get("gui/settingsMenu/infoPrompt/promptBg.png", Texture.class);

		infoLayout.setText(infoFont, message, Color.WHITE,
				backgroundImage.getWidth() - 20, Align.center, true);
	}
	...
	public void update(float delta) {
		...
		/* Position background */
		bgX = MainCamera.getInstance().getCenterX()
				- (backgroundImage.getWidth() / 2);
		bgY = MainCamera.getInstance().getCenterY()
...
class InfoAcceptButton extends ButtonComponent {
	...
	public InfoAcceptButton(InfoPrompt prompt) {
		...
		this.setImage(GameManager.getInstance().assetManager.get(
				"gui/settingsMenu/infoPrompt/acceptButton.png", Texture.class));
		...
		this.setImagePushed(GameManager.getInstance().assetManager.get(
				"gui/settingsMenu/infoPrompt/acceptButtonPushed.png",
				Texture.class));
	}
