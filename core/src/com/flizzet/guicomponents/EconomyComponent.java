...
public class EconomyComponent extends GuiComponent {
	...
	public EconomyComponent() {
		...
		this.billIcon = GameManager.getInstance().assetManager
				.get("gui/icons/billIcon.png", Texture.class);
	...
	public void update(float delta) {
		...
		coinX = bounds.x + MainCamera.getInstance().getWidth() - margin
				- coinWidth;
		coinY = bounds.y + MainCamera.getInstance().getHeight() - margin
				- coinHeight;

		/* Platform icon positioning */
		billX = bounds.x + margin;
		billY = bounds.y + MainCamera.getInstance().getHeight() - margin
				- billHeight;

		/* Add money button */
		addMoneyButton.setX(bounds.x + MainCamera.getInstance().getCenterX()
				- (addMoneyButton.getWidth() / 2));
		addMoneyButton.setY(
				billY + (billHeight / 2) - (addMoneyButton.getHeight() / 2));
	...
	public void render(SpriteBatch batch) {
		...
		layout.setText(font, String
				.valueOf(Player.getInstance().getScore().getTotalBills()));
		font.setUseIntegerPositions(false);
		font.draw(batch,
				String.valueOf(Player.getInstance().getScore().getTotalBills()),
				billX + billWidth + textSpacing,
				billY + (billHeight / 2) + (font.getCapHeight() / 2));

		/* Coins */
		layout.setText(font, String
				.valueOf(Player.getInstance().getScore().getTotalCoins()));
		font.draw(batch,
				String.valueOf(Player.getInstance().getScore().getTotalCoins()),
				coinX - layout.width - textSpacing,
