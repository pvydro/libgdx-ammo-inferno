...
public class ReviveButton extends ButtonComponent {
	...
	private Texture billIcon = GameManager.getInstance().assetManager
			.get("gui/icons/billIcon.png");
	...
	public void update(float delta) {
		...
		this.subtextX = this.getCenterX() + (fullWidth / 2)
				- (subtextLayout.width);
		billX = this.getCenterX() - (fullWidth / 2);
		billY = subtextY - (subtextLayout.height / 2)
				- (billIcon.getHeight() / 2);
	}
	...
	public void triggered() {
		Platform newPlatform = PlatformGenerator.getInstance().getPlatforms()
				.get((int) ((PlatformGenerator.getInstance().getPlatforms()
						.size() - 1) / 2));
		float newY = newPlatform.getY() + newPlatform.getHeight() + 10;
		Player.getInstance().setY(newY);
		Player.getInstance().setX(newPlatform.getCenterX()
