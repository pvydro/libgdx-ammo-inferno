...
class Screen extends Entity {
	...
	private GlyphLayout textLayout = new GlyphLayout(textFont,
	...
	public void update(float delta) {
		...
		bounds.y = MainCamera.getInstance().getHeight() - bounds.height
				- MARGIN;

		/* Position text */
		textLayout.setText(textFont, currentCategory.getMessage(), Color.WHITE,
				bounds.width - 10, Align.center, true);
		textX = bounds.x + 5;
	...
	/*
	 * Moves to the next category if there is one, otherwise enter the start
	 * menu
	 */
	public void nextCategory() {
		if (currentCategory.ordinal() < ScreenCategory.values().length - 1) {
			currentCategory = ScreenCategory.values()[currentCategory.ordinal()
					+ 1];
		} else {
