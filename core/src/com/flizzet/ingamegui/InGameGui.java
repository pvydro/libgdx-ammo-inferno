...
public class InGameGui {
	...
	public void buildGui() {
		...
		if (CurrentSettings
				.getInstance().controls == ControlType.HORIZONTALARROWS) {

			jumpButton.setX(MainCamera.getInstance().getCenterX()
					- (jumpButton.getWidth() / 2));
			jumpButton.setY(MARGIN + jumpButton.getHeight() - 3);
			moveRightButton
					.setX(jumpButton.getX() + moveRightButton.getWidth());
			moveRightButton.setY(MARGIN);
			moveLeftButton.setX(jumpButton.getX());
			moveLeftButton.setY(MARGIN);
			targetButton.setX(MainCamera.getInstance().getWidth()
					- targetButton.getWidth() - MARGIN);
			targetButton.setY(MARGIN);

		} else if (CurrentSettings
				.getInstance().controls == ControlType.CONSOLE) {

			float fullWidth = moveLeftButton.getWidth()
					+ moveRightButton.getWidth() + targetButton.getWidth()
					+ jumpButton.getWidth();
			float left = MainCamera.getInstance().getCenterX()
					- (fullWidth / 2);
			float right = MainCamera.getInstance().getCenterX()
					+ (fullWidth / 2);
			...
			float distanceBetweenJumpAndRight = jumpButton.getX()
					- moveRightButton.getX(); // HERE GO with the LOOOONG
												// VARIABLE NAME YUHHH COPY
												// PASTE GANG
			targetButton.setX(
					moveRightButton.getX() + (distanceBetweenJumpAndRight / 2));
			/* Y */
			...
			GameManager.getInstance().guiContainer
					.removeFromGui(moveLeftButton);
			GameManager.getInstance().guiContainer
					.removeFromGui(moveRightButton);

			touchpad = new TouchpadJoystick();
			touchpad.getTouchpad().setBounds(left, margin, 72.0f, 72.0f);
			GameManager.getInstance().getStage()
					.addActor(touchpad.getTouchpad());

		} else if (CurrentSettings
				.getInstance().controls == ControlType.JOYSTICK) {
			joystick = new JoystickComponent();
			joystick.setX(MainCamera.getInstance().getCenterX()
					- (joystick.getWidth() / 2));
			joystick.setY(MARGIN);
			targetButton.setX(MainCamera.getInstance().getWidth() - MARGIN
			...
			targetButton.setX(MainCamera.getInstance().getCenterX()
					- (targetButton.getWidth() / 2));
			targetButton.setY(MARGIN);
		}
	...
	public TouchpadJoystick getTouchpad() {
