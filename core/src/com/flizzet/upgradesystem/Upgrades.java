...
public class Upgrades {
	 ...
	 *             Upgrades class is already loaded.
	 */
	public void loadUpgrades() throws AlreadyLoadedException {
		...
		upgrades.add(new StyrofoamDartsUpgrade()); // 0
		upgrades.add(new EarlyBirdUpgrade()); // 1
		upgrades.add(new LuckyDogUpgrade()); // 2
		upgrades.add(new DoubleJumpUpgrade()); // 3
		upgrades.add(new ZombieLooterUpgrade()); // 4
		upgrades.add(new ExcavationistUpgrade()); // 5
		upgrades.add(new DinkUpgrade()); // 6
		upgrades.add(new ExtraLifeUpgrade()); // 7
		upgrades.add(new TrapJammerUpgrade()); // 8
		upgrades.add(new BandaidUpgrade()); // 9
		upgrades.add(new SwiftUpgrade()); // 10
		upgrades.add(new LavaBootsUpgrade()); // 11
	}
	 ...
	 *             The upgradeType (E) is not in the Upgrades ArrayList
	 */
	public <E> boolean isBought(Class<E> upgradeType) {
		...
		throw new NullPointerException("Upgrade type " + upgradeType
				+ " is not in the Upgrades ArrayList");
	}
				...
				if (!u.isBought()) {
					if (pay) {
						Player.getInstance().getScore()
								.removeFromCurrentCoins(u.getPrice());
						u.setBought(true);
					} else {
						u.setBought(true);
					}
	 ...
	 *            The upgrade type
	 * @param slot
	 *            A number between 1 and 3, defines the index of the upgrade
	 */
