...
public class Heads {
	 ...
	 *             The upgradeType (E) is not in the Heads ArrayList
	 */
	public <E> boolean isBought(Class<E> headType) {
		...
		throw new NullPointerException(
				"Head type " + headType + " is not in the Heads ArrayList");
	}
	 ...
	 *            the upgrade type
	 * @throws NullPointerException
	 *             The headType (E) is not in the Heads ArrayList
	 */
				...
				if (!h.isBought()) {
					if (pay) {
						Player.getInstance().getScore()
								.removeFromCurrentCoins(h.getPrice());
						h.setBought(true);
					} else {
						h.setBought(true);
					}
