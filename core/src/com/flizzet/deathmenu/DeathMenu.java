...
public class DeathMenu extends GuiComponent {
	...
	private GlyphLayout largeLayout = new GlyphLayout(fontLarge,
			String.valueOf(Player.getInstance().getScore().getPlatforms()));

	private float platformsScoreX = MainCamera.getInstance().getCenterX()
			- (largeLayout.width / 2);
	private float platformsTextX = MainCamera.getInstance().getCenterX()
			- (mediumLayout.width / 2);
	private float platformsHighTextX = MainCamera.getInstance().getCenterX()
			- (smallLayout.width / 2);
	...
	public void render(SpriteBatch batch) {
		...
		if (built) {
			...
			/* Score text */
			largeLayout.setText(fontLarge, String
					.valueOf(Player.getInstance().getScore().getPlatforms()));
			fontLarge.draw(batch,
					String.valueOf(
							Player.getInstance().getScore().getPlatforms()),
					platformsScoreX, MainCamera.getInstance().getHeight() - 8
							- largeLayout.height);
			fontMedium.draw(batch, "platforms", platformsTextX,
					MainCamera.getInstance().getHeight() - 26
							- largeLayout.height - mediumLayout.height);
			fontSmall.draw(batch,
					"high: " + String.valueOf(
							Player.getInstance().getScore().getHighScore()),
					platformsHighTextX,
					MainCamera.getInstance().getHeight() - builder.getMargin()
							- 33 - largeLayout.height - mediumLayout.height
							- smallLayout.height);
	...
	public void enable() {
		...
		largeLayout = new GlyphLayout(fontLarge,
				String.valueOf(Player.getInstance().getScore().getPlatforms()));
		smallLayout = new GlyphLayout(fontSmall, "high: " + String
				.valueOf(Player.getInstance().getScore().getHighScore()));
		shopSelected = false;
		platformsScoreX = MainCamera.getInstance().getCenterX()
				- (largeLayout.width / 2);
		platformsTextX = MainCamera.getInstance().getCenterX()
				- (mediumLayout.width / 2);
		platformsHighTextX = MainCamera.getInstance().getCenterX()
				- (smallLayout.width / 2);
	...
	public DeathMenuBuilder getBuilder() {
		return this.builder;
	}
	...
	public void setShopSelected(boolean isSelected) {
		this.shopSelected = isSelected;
	}
