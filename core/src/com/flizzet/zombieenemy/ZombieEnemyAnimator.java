...
class ZombieEnemyAnimator implements Renderable {
	...
	public ZombieEnemyAnimator(ZombieEnemy zombie) {
		...
		Texture walkingSheet = GameManager.getInstance().assetManager
				.get("enemies/zombie/body/bodyWalking.png", Texture.class);
		Texture deathSheet = GameManager.getInstance().assetManager
				.get("enemies/zombie/body/bodyDying.png", Texture.class);
	...
	public void render(SpriteBatch batch) {
		...
		/* Draw currentframe */
		batch.draw(currentFrame, zombie.getX(), zombie.getY(),
		...
		if (CurrentSettings.getInstance().showCollisions) {
			...
			shapeRenderer.rect(zombie.getCollisionBounds().x,
					zombie.getCollisionBounds().y,
					zombie.getCollisionBounds().width,
					zombie.getCollisionBounds().height);
			shapeRenderer.end();
			batch.begin();
		}
