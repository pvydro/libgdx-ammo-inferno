...
public class ZombieEnemyDialogueGenerator implements Updatable, Renderable {
	...
	public void render(SpriteBatch batch) {
		if (messageChosen) {
			...
			font.draw(batch, message, enemy.getCenterX() - (layout.width / 2),
					enemy.getY() + enemy.getHeight() + 30);
		}
	}
	...
	private void speak() {
		messageChosen = true;
		FileHandle file = Gdx.files.internal("enemies/zombie/zombie.dia"); // Get
																			// the
																			// dialogue
																			// file
		...
		int choice = random.nextInt(choices.length); // Choose a random option
														// from the file
