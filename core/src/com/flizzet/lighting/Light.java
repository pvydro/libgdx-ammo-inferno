...
abstract class Light extends Entity {
	...
	/**
	 * Default instantiable constructor Takes a Texture as a light mask
	 */
	public Light() {
		hardLightImage = GameManager.getInstance().assetManager
				.get("lights/hardLight.png", Texture.class);
	}
	...
	public void update(float delta) {
		...
		if (jitterHorizontally) {
			/* Set jitter position */
			if (jitterPosition) {
				jitterX = random.nextBoolean()
						? random.nextInt(jitterPositionAmt)
						: -random.nextInt(jitterPositionAmt);
			}
			/* Set jitter dimension */
			if (jitterDimension) {
				jitterWidth = random.nextBoolean()
						? random.nextInt(jitterDimensionAmt)
						: -random.nextInt(jitterDimensionAmt);
			}
		} else {
			jitterX = 0;
	...
	public void render(SpriteBatch batch) {
		...
		if (CurrentSettings.getInstance().softLight) {
			batch.draw(lightOverlay, bounds.x + jitterX, bounds.y + jitterY,
					bounds.width + jitterWidth, bounds.height + jitterHeight);

		} else if (!CurrentSettings.getInstance().softLight) {
			batch.draw(hardLightImage, bounds.x + jitterX, bounds.y + jitterY,
					bounds.width + jitterWidth, bounds.height + jitterHeight);
		}
