...
public class SuperCaveJumper extends ApplicationAdapter {
	...
	public void create() {
		...
		batch.setProjectionMatrix(
				MainCamera.getInstance().getCamera().combined); // Set projection to camera
		gameManager.stateManager.addGameState(
				new LoadingState(gameManager.stateId_loading, gameManager)); // Add the game states
		gameManager.stateManager.addGameState(
				new SplashState(gameManager.stateId_splash, gameManager));
		gameManager.stateManager.addGameState(
				new StartMenuState(gameManager.stateId_start, gameManager));
		gameManager.stateManager.addGameState(
				new PlayState(gameManager.stateId_play, gameManager));
		gameManager.stateManager.addGameState(
				new HeadShopState(gameManager.stateId_headShop, gameManager));
		gameManager.stateManager.addGameState(new WeaponShopState(
				gameManager.stateId_weaponShop, gameManager));
		gameManager.stateManager.addGameState(new UpgradeShopState(
				gameManager.stateId_upgradeShop, gameManager));
		gameManager.stateManager.addGameState(
				new SettingsState(gameManager.stateId_settings, gameManager));
		gameManager.stateManager.addGameState(new ControlsSettingsState(
				gameManager.stateId_controlsSettings, gameManager));
		gameManager.stateManager.addGameState(new EffectsSettingsState(
				gameManager.stateId_effectsSettings, gameManager));
		gameManager.stateManager.addGameState(new GeneralSettingsState(
				gameManager.stateId_generalSettings, gameManager));
		gameManager.stateManager.addGameState(
				new ShopState(gameManager.stateId_shop, gameManager));
		gameManager.stateManager.addGameState(
				new CreditsState(gameManager.stateId_credits, gameManager));
		gameManager.stateManager.addGameState(
				new TutorialState(gameManager.stateId_tutorial, gameManager));
	...
	public void render() {
		...
		switch (pauseManager.getState()) {
			case PLAY:
				...
				GameManager.getInstance().entityContainer
						.update(Gdx.graphics.getDeltaTime());
				GameManager.getInstance().particleContainer
						.update(Gdx.graphics.getDeltaTime());
				...
				GameManager.getInstance().getCamera()
						.update(Gdx.graphics.getDeltaTime());
				batch.setProjectionMatrix(GameManager.getInstance().getCamera()
						.getCombinedMatrix());
				...
				if (GameManager.getInstance().stateManager
					...
					MainCamera.getInstance().getCamera().position.y += 10
							* ((MainCamera.getInstance().getTargetY()
									- MainCamera.getInstance().getCamera().position.y) / 2)
							* Gdx.graphics.getDeltaTime();
					MainCamera.getInstance().getCamera().position.x += 10
							* ((MainCamera.getInstance().getTargetX()
									- MainCamera.getInstance().getCamera().position.x) / 2)
							* Gdx.graphics.getDeltaTime();
				}
				break;
			case SELECT:
				...
				for (Entity e : GameManager.getInstance().entityContainer
						.getEntities()) {
				...
				Player.getInstance().getWeapon()
						.rotate(Gdx.graphics.getDeltaTime());

				MainCamera.getInstance().getCamera().position.y += 10
						* ((MainCamera.getInstance().getTargetY() - MainCamera
								.getInstance().getCamera().position.y) / 2)
						* Gdx.graphics.getDeltaTime();
				MainCamera.getInstance().getCamera().position.x += 10
						* ((MainCamera.getInstance().getTargetX() - MainCamera
								.getInstance().getCamera().position.x) / 2)
						* Gdx.graphics.getDeltaTime();
				break;
			case REVIVE:
			case PAUSE:
		...
		MainCamera.getInstance().getCamera().translate(
				MainCamera.getInstance().getXShakeAmt(),
				MainCamera.getInstance().getYShakeAmt());
		...
		if (!GameManager.getInstance().ambience.isAboveGui())
			GameManager.getInstance().ambience.render(batch);
		...
		if (GameManager.getInstance().assetManager.update()) {
			SelectTargetScreen.getInstance()
					.update(Gdx.graphics.getDeltaTime());
		...
		for (Actor a : GameManager.getInstance().getStage().getActors()) {
			currentX = a.getX();
			currentY = a.getY();
			float shakeXOffset = (MainCamera.getInstance()
					.getCamera().position.x
					- MainCamera.getInstance().getTargetX());
			float shakeYOffset = (MainCamera.getInstance()
		...
		if (GameManager.getInstance().stateManager
				.getCurrentState() != GameManager.getInstance().stateId_play
				&& GameManager.getInstance().stateManager
						.getCurrentState() != GameManager.getInstance().stateId_tutorial
				&& GameManager.getInstance().stateManager
						.getCurrentState() != GameManager.getInstance().stateId_loading) {
			if (!MusicHandler.getInstance().getMenuMusic().isPlaying()) {
				MusicHandler.getInstance()
						.play(MusicHandler.getInstance().getMenuMusic(), true);
			}
		} else if (MusicHandler.getInstance().getMenuMusic() != null) {
			if (MusicHandler.getInstance().getMenuMusic().isPlaying()) {
				MusicHandler.getInstance()
						.stop(MusicHandler.getInstance().getMenuMusic());
			}
		}
