 ...
 * @version 1.0
 */
public class EffectsSettingsMenuBuilder extends MenuBuilder
		implements
			Updatable {
	...
	public void buildMenu() {
		...
		separators.add(
				particlesSeparator = new SeparatorComponent(2, "particles"));
		...
		/*
		 * The background must be added before this so the checkboxes draw above
		 * it
		 */
		...
		/*
		 * Add checkbox to ArrayList Effects
		 */
		...
		for (CheckboxComponent c : effectCheckboxes) {
			...
			c.setY(effectsSeparator.getY() - 3
					- (c.getHeight() + 5) * totalEffectsCheckboxes);

			GameManager.getInstance().guiContainer.addToGui(c);
		}

		// Separate
		particlesSeparator.setY(
				effectCheckboxes.get(effectCheckboxes.size() - 1).getY() - 13);
		...
		lightParticlesCheckbox = new CheckboxComponent("light",
			...
			public void triggered() {
				switch (state) {
					case ON:
						CurrentSettings
								.getInstance().platformLightParticles = true;
						break;
					case OFF:
						CurrentSettings
								.getInstance().platformLightParticles = false;
						break;
				}
			}
		};
		...
		// Separate
		finalSeparator.setY(
				particleCheckboxes.get(particleCheckboxes.size() - 1).getY()
						- 10);
	...
	public ArrayList<CheckboxComponent> getEffectCheckboxes() {
		return this.effectCheckboxes;
	}
	public ArrayList<CheckboxComponent> getParticleCheckboxes() {
		return this.particleCheckboxes;
	}
	public ArrayList<SeparatorComponent> getSeparators() {
		return this.separators;
	}
	public SeparatorFillerComponent getFiller() {
		return this.filler;
	}
