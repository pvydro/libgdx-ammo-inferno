...
public class SelectTargetScreen extends GuiComponent {
	...
	public void update(float delta) {
		...
		if (direction == 0) {
			firstLinePos.y += ((MainCamera.getInstance().getHeight()
					- lineWidth) - firstLinePos.y) / 50;
			if (firstLinePos.y > MainCamera.getInstance().getHeight()
					- lineWidth - 2) {
				direction = 1;
			}
		} else {
	...
	public void render(SpriteBatch batch) {
		if (alpha > 0.01f) {
			...
			shapeRenderer.setColor(new Color(Color.MAROON.r, Color.MAROON.g,
					Color.MAROON.b, alpha / 10));
			shapeRenderer.rect(-100, -100,
					MainCamera.getInstance().getWidth() + 200,
					MainCamera.getInstance().getHeight() + 200);
			...
			for (int i = 0; i < totalRects; i++) {
				float rectX = random
						.nextInt((int) MainCamera.getInstance().getWidth());
				float rectY = random
						.nextInt((int) MainCamera.getInstance().getHeight());
